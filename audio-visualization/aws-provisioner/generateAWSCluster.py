...
def initializeRefData():
    ...
    ramInfo = [('t3.micro',2),('m4.large',8),('m4.xlarge',16),('m4.2xlarge',32),('m4.4xlarge',64),('r4.xlarge',30),('r4.2xlarge',61),('r4.4xlarge',122)]
...
def validate(ctx):
    ...
    for index, server in enumerate(ctx['Servers']):
        ...
        if  server['InstanceType'] not in instanceProps:
            sys.exit('server {0} failed a validation: "InstanceType" must be one of the following - {1}'.format(index,','.join(instanceProps.keys())))
